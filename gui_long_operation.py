...
def main():
    ...
    layout = [  [sg.Text('My Window')],
                [sg.Input(key='-IN-')],
                [sg.Text(key='-OUT-')],
                [sg.Button('Go'), sg.Button('Thread'), sg.Button('Print')]  ]
    ...
    while True:             # Event Loop
        ...
        if event == sg.WIN_CLOSED:
            break
        if event == 'Go':
            return_value = my_long_operation('Go completed')
            window['-OUT-'].update(f'Go return value = {return_value}')
        elif event  == 'Thread':
            # Let PySimpleGUI do the threading for you...
            window.perform_long_operation(lambda: my_long_operation('Thread completed'), '-OPERATION DONE-')
