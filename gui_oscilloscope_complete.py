...
layout = [  [sg.Text('Channel', size=text_size), sg.Combo(channels, default_value=channels[0], key='-channel-', expand_x=True)],
            ...
            [sg.Text('Timebase position', size=text_size), sg.Slider(range=(-5.0, 5.0), default_value=0.0, resolution=0.1, orientation='h', key='-tposition-', expand_x=True)],
...
while True:
    ...
    if event == 'Connect':  # If the user clicks the Connect button, enable the send button
        ...
        sg.popup(f"Channel: {values['-channel-']}\nVertical scale: {values['-vscale-']}\nVertical offset: {values['-voffset-']}\nTimebase scale: {values['-tscale-']}\nTimebase position: {values['-tposition-']}")
